...
istream &inpDT(istream &in, Date_Time &DT){
	in>>DT.Day;
	in>>DT.Month;
	in>>DT.Year;
	in>>DT.Hour;
...
ostream &outDT(ostream &out, const Date_Time &DT){
	out<<DT.Day<<endl<<DT.Month<<endl<<DT.Year<<endl<<DT.Hour<<endl<<DT.Minute<<endl;
	return out;
}
